...
def copy_files(src, dest, ignoredExt = None, ignoredFiles = None):
    if not os.path.exists(src):
        log_utils.warning("the path is not exists.path:%s", src)
        return

    filename = os.path.basename(src)
    print("the base name of ignoredFiles:"+filename)
    if ignoredFiles != None and filename in ignoredFiles:
        return

    if os.path.isfile(src):
        copy_file(src, dest, ignoredExt)
        return
    ...
    for f in os.listdir(src):
        ...
        #     copy_file(sourcefile, targetfile, ignoredExt)
        # else:
        #     copy_files(sourcefile, targetfile, ignoredExt)
...
if __name__ == '__main__':
	...
	copy_files(".", "./target", None, ["U8_Temp_AS_Project"])
