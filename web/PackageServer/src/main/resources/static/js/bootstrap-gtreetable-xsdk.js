...
(function ($) {
    ...
    function GTreeTable(element, options) {
        ...
        this.template = this.options.template !== undefined ? this.options.template : 
           ...
           '<tr class="' + this.options.classes.node + ' ">' +
           '<td>' +
           // '<div class="btn-group check-item"><input type="checkbox" class="tree-item-checkbox" /></div>' +
      ...
      this.templateParts = this.options.templateParts !== undefined ? this.options.templateParts :
            {
                ...
                actionsList: '',
                selectState:'<div class="btn-group check-item"><input type="checkbox" class="tree-item-checkbox" /></div>'
            };
    ...
    GTreeTable.prototype = {
        ...
        updateNodeName:function(params){
            var node = this.getNodeById(params.id);
            if(node !== undefined){
                node.renderName(params.name);
            }
        },
        ...
        clear:function () {
            ...
            $.each(this.$tree.find('.' + this.options.classes.node), function () {
                var oNode = that.getNode($(this));
                oNode.remove();
            });
            that.nodeData = [];
        ...
        getCheckedNodes:function () {
          var that = this;
          var nodes = that.nodeData;

          var result = [];
          for(var nodeIndex in nodes){
              if(nodes[nodeIndex].isChecked()){
        ...
        init: function () {
            ...
            var that = this;
            that.clear();
            this.getSourceNodes(0, false).done(function (result) {
                ...
                for(var x in data) {
                    ...
                    that.nodeData.push(oNewNode);
                    //added by chenjie, to support expanded tree of all children nodes
                    var children = data[x][that.options.nodesWrapper];
                    if(children != null){
                        var prevNode = oNewNode;
                        for(var y in children) {
                            index = index + 1;
                            children[y].parent = data[x].id;
                            children[y].index = index;
                            var newNode = new GTreeTableNode(children[y], that);
                            oNewNode.insertIntegral(newNode, prevNode);
                            that.nodeData.push(newNode);
                            prevNode = newNode;
                        }
                    }
    ...
    function GTreeTableNode(data, gtreetable) {
        ...
        this.type = data.type;
        this.id = data.id;
        this.selectState = parseInt(data.selectState);
    ...
    GTreeTableNode.prototype = {
        ...
        getDirectParent:function () {
            ...
            return this.manager.getNodeById(parentId);
        },
        ...
        changeCheckState:function (checked) {
            ...
            oNode.$checkbox.prop("checked",checked);
        },
        ...
        init: function () {
            ...
            this.$checkbox = this.$node.find('.'+ 'tree-item-checkbox');
        ...
        renderName: function(name){
            this.name = name;
            if(this.level == 0){
                this.$name.html('<i class="fa fa-folder-open-o" style="padding-right: 3px;"></i>'+'<span style="font-weight: bold;">' + name + '</span>');
            }
            else{
                this.$name.html('<i class="fa fa-file-o" style="padding-right: 3px;"></i>'+name);
            }
        },
        ...
        render: function() {
            ...
            //if(this.level == 0){
            //    this.$name.html('<i class="fa fa-folder-open-o" style="padding-right: 3px;"></i>'+'<span style="font-weight: bold;">' + this.name + '</span>');
            //}
            //else{
            //    this.$name.html('<i class="fa fa-file-o" style="padding-right: 3px;"></i>'+this.name);
            //}
            ...
            //add odd class
            if(this.index % 2 == 1){
                this.$node.addClass(this.manager.options.classes.odd);
            }
        ...
        attachEvents: function () {
            ...
            //append checkbox event. --by chenjie
            this.$checkbox.change(function () {
                ...
                var children = that.getDescendants({depth: 1, includeNotSaved: true});

                if(children.length > 0){
                    for(var nodeIndex in children){
                        var cNode = children[nodeIndex];
                        cNode.changeCheckState(checked);
                    }
                }

                if ($.isFunction(that.manager.options.onCheckStateChanged)) {
                    var checkedItems = that.manager.getCheckedNodes();
                    that.manager.options.onCheckStateChanged(that, checkedItems);
                }

                //alert("hahaha"+that.getId()+';len:'+checkedItems.length);
            });
        ...
        _remove: function () {            
            ...
            if (this.parent > 0) {
                ...
                if (oParent != null && oParent !== undefined && oParent.getDescendants({ depth: 1, includeNotSaved: true }).length === 0) {
                    oParent.collapse();
                }
            }
        ...
        isSelected: function (action) {
            if (action === undefined) {
                ...
                this.$node.removeClass(this.manager.options.classes.odd);
   ...
   function GTreeTableCache(manager) {
    ...
    GTreeTableCache.prototype = {  
        ...
        _synchronizeDelete: function(oNode) {            
            ...
            if(oParentNode === undefined) return;
